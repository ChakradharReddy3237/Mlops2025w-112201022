...
df = df.iloc[1000:2000]  # use 1000 rows for speed
...
def build_txn_docs(df: pd.DataFrame):
    docs = []
...
def build_cust_docs(df: pd.DataFrame):
    docs = []
...
t1 = avg_exec_time(
    lambda: list(txn_col.aggregate([{ "$group": {"_id": "$country", "total": {"$sum": "$total_amount"}} }])),
    runs=10,
)
t2 = avg_exec_time(
    lambda: list(cust_col.aggregate([{ "$group": {"_id": "$country", "total": {"$sum": "$total_spent"}} }])),
    runs=10,
)
res.append(("READ - Sales by Country", t1, t2))
...
# transaction count
t1 = avg_exec_time(
    lambda: list(txn_col.aggregate([{ "$group": {"_id": "$customer_id", "cnt": {"$sum": 1}} }])),
    runs=10,
)
t2 = avg_exec_time(
    lambda: list(cust_col.aggregate([{ "$project": {"customer_id": 1, "cnt": {"$size": "$invoices"}} }])),
    runs=10,
)
...
# increment totals for a customer
...
# increment totals by country
t1 = avg_exec_time(txn_col.update_many, runs=5, **{"filter": {"country": test_country}, "update": {"$inc": {"total_amount": 1}}})
t2 = avg_exec_time(cust_col.update_many, runs=5, **{"filter": {"country": test_country}, "update": {"$inc": {"total_spent": 1}}})
res.append(("UPDATE - By Country", t1, t2))
...
# Save numeric results to CSV (simple differentiation)
import csv
with open("crud_benchmark_results.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["operation", "txn_centric", "cust_centric"])
    for op, a, b in res:
        writer.writerow([op, f"{a:.6f}", f"{b:.6f}"])
...
plt.legend()
plt.tight_layout()
plt.savefig("crud_benchmark.png", dpi=150)
try:
    plt.show()
except Exception:
    pass
